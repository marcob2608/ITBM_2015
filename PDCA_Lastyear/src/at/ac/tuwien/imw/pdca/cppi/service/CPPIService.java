...
public class CPPIService {
    ...
    private BigDecimal prevpreviousStockPrice;
    ...
    private ArrayList<CPPIValues> cppiValuesHistory = new ArrayList<CPPIValues>();

    private CPPIService() {
    }

    public synchronized boolean addCPPIValues(CPPIValues values) {
        if (!cppiValuesHistory.contains(values)) {
            return cppiValuesHistory.add(values);
        }
        return false;
    }

    public synchronized CPPIValues getCppiValues(int period) {
        return cppiValuesHistory.get(period);
    }

    public int getCPPICount() {
        return cppiValuesHistory.size();
    }
    ...
    public void init() {
        ...
        prevpreviousStockPrice = new BigDecimal(100);
    ...
    public synchronized void updateActualStockPrice() {
        //      log.info("updateActualStockPrice");
        currentPeriod++;
        prevpreviousStockPrice = previousStockPrice;
        previousStockPrice = new BigDecimal(currentStockPrice.doubleValue());
        currentStockPrice = new BigDecimal(stockPrices.get((currentPeriod-1) % stockPrices.size()));
    ...
    public BigDecimal getPrevpreviousStockPrice() {
        return prevpreviousStockPrice;
    }
}
